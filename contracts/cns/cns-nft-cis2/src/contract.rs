...
/// list of standards supported by this contract.
const SUPPORTED_STANDARDS: [StandardIdentifier<'static>; 2] =
...
fn mint<S: HasStateApi>(
    ...
    let url = build_token_metadata_url(&params.token_id);
    // Event for Metadata URL update.
    logger.log(&Cis2Event::TokenMetadata::<_, TokenAmountU64>(
        TokenMetadataEvent {
...
fn update_operator<S: HasStateApi>(
    ...
    for param in params {
        ...
        logger.log(
            &Cis2Event::<ContractTokenId, TokenAmountU64>::UpdateOperator(UpdateOperatorEvent {
                owner: sender,
...
#[receive(
    ...
    return_value = "BalanceOfQueryResponse<TokenAmountU64>"
)]
fn balance_of<S: HasStateApi>(
    ctx: &impl HasReceiveContext,
    host: &impl HasHost<State<S>, StateApiType = S>,
) -> ContractResult<BalanceOfQueryResponse<TokenAmountU64>> {
    ...
    for query in params.queries {
        ...
        let amount = TokenAmountU64(u64::from(
...
fn contract_token_metadata<S: HasStateApi>(
    ...
    // Build the response.
    let mut response = Vec::with_capacity(params.queries.len());
    let token_storage = host.state().token_storage;
    let slot_time = ctx.metadata().slot_time();

    for token_id in params.queries {
        // Ensure the token exists and is not expired.
        ensure!(
            !storage::get_token_subscription_data(host, &token_storage, &token_id)?
                .ok_or(ContractError::InvalidTokenId)?
...
fn set_implementor<S: HasStateApi>(
    ctx: &impl HasReceiveContext,
    host: &mut impl HasHost<State<S>, StateApiType = S>,
) -> ContractResult<()> {
    if !host.state().authority.has_maintainer_rights(&ctx.sender()) {
    ...
    fn test_transfer_to_contract() {
        ...
        let params = TransferParams(vec![Transfer {
            ...
            to: Receiver::Contract(
                CONTRACT_1,
                OwnedEntrypointName::new_unchecked("Listing.list".into()),
            ),
        ...
        host.setup_mock_entrypoint(
            ...
            parse_and_check_mock::<OnReceivingCis2Params<TokenIdVec, _>, _>(
